    ...
    effectivePom(item) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const ret = yield vscode_1.window.withProgress({ location: vscode_1.ProgressLocation.Window }, (p) => new Promise((resolve, reject) => {
                ...
                child_process_1.exec(cmd, { cwd: rootfolder ? rootfolder.uri.fsPath : path.dirname(pomXmlFilePath) }, (error, _stdout, _stderr) => {
                    if (error) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (selectedGoal === ENTRY_NEW_GOALS) {
                ...
                if (trimedGoals) {
                    ...
                    VSCodeUI_1.VSCodeUI.runInTerminal([
                        Utils_1.Utils.getMavenExecutable(),
                        trimedGoals,
                        "-f",
                        `"${item.abosolutePath}"`
                    ].join(" "), { name: `Maven-${item.artifactId}` });
                }
            }
